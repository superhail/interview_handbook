...
- HTTP 2.0:   **二进制分帧**，其中，http1.X中的首部信息header封装到Headers帧中，而request body将被封装到Data帧中，**解决了头部阻塞问题**；**多路复用**(Multiplexing)/连接共享，文件载荷被切割；**头部压缩**；**请求优先级**；**服务端推送**

- HTTP 3.0: 基于google的QUIC协议，而quic协议是使用udp实现的；解决了网络迁移需要重新连接的问题；解决了头部阻塞问题
